...
uploadCnrRoute.post("/upload-cnr-numbers", async (req, res) => {
    try {
      ...
      if (!Array.isArray(cnrNumbers) || !cnrNumbers.length) {
        return res.status(400).json({ error: "cnrNumbers must be a non-empty array." });
      }
      ...
      for (const cnrNumber of cnrNumbers) {
        ...
        // Find the existing entry for the CNR number
      ...
      res.status(200).json({ status: true, message: "CNR numbers stored successfully.", savedEntries });
    } catch (error) {
      console.error("Error saving CNR numbers:", error.message);
      res.status(500).json({ status: false, error: "An error occurred while storing CNR numbers." });
    }
  });
