...
export const addMoreDocument = async (req, res) => {
  try {
    const { cnrNumber, id } = req.body;
    const { token } = req.headers;
    ...
    document.documents = [...document.documents, ...attachments];
    document.noOfDocument = document.documents.length;
    await document.save();
    return res.json({
      message: "Documents uploaded successfully",
      success: true,
...
export const getDocument = async (req, res) => {
  ...
  try {
    ...
    const pageSize = Math.ceil(documents.length / parseInt(pageLimit));
    return res.json({
...
export const deleteDocument = async (req, res) => {
  ...
  try {
    ...
    if (document.noOfDocument === 1) {
      const ndocument = await Document.deleteOne({
        cnrNumber,
        userId: decodedToken.id,
      });
...
export const getDocumentOfSingleSubCnr = async (req, res) => {
  const { token } = req.headers;
